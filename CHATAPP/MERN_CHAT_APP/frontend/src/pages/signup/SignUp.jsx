    ...
    <div className="flex flex-col justify-center min-w-96 max-auto items-center">
        <div className="w-full p-6 rounded-lg shadow-md bg-gray-400 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-0">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <Link to="/login" className="text-sm hover:underline hover:font-bold  mt-2 inline-block">
                    {"Already"} have an account?
                </Link>
                <div>
                    <button className="bg-zinc-800 hover:text-gray-900 text-white btn btn-block btn-sm mt-2" disabled={loading} >{loading ? <span className="loading loading-spinner"></span> : "Sign Up"}</button>
                </div>
            </form>
